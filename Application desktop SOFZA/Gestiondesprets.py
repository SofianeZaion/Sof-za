...
class gestionprets:  # classe 
    def __init__(self,root):  #constructeur                  
        ...
        self.Paneaugrisdegestionlivres = Frame(self.PageGestiondesprets, bg="#9D7B7F")
        self.Paneaugrisdegestionlivres.place(x=190, y=0, width=1100, height=1000)

        Paneaumarrondegestionlivres = Frame(self.PageGestiondesprets, bg="#8E7E23")
        Paneaumarrondegestionlivres.place(x=0, y=0, width=190, height=1000)


        self.ImageGestionlivres = PhotoImage(file="Images/Gestionlivre.png")
        self.BoutonPourAllerVersGestionLivres = Button(self.PageGestiondesprets,cursor="hand2",command=self.VersGestionLivres, text="",image=self.ImageGestionlivres, width=184,height=90, bg="#8E7E23",font="arial 12 bold")
        self.BoutonPourAllerVersGestionLivres.place(x=0 , y=0) 
        
        self.ImageAdherents = PhotoImage(file="Images/Adherents.png")
        self.BoutonPourAllerVersAdherents = Button(self.PageGestiondesprets,cursor="hand2",command=self.VersAdherents, text="",image=self.ImageAdherents, width=184, height=90, bg="#8E7E23",font="arial 12 bold")
        self.BoutonPourAllerVersAdherents.place(x=0 , y=140) 

        self.ImageGestionDesprets = PhotoImage(file="Images/Emprunter.png")
        self.BoutonPourAllerVersGestionDesprets = Button(self.PageGestiondesprets, cursor="hand2",text="",image=self.ImageGestionDesprets, width=184, height=90, bg="#8E7E23",font="arial 12 bold")
        self.BoutonPourAllerVersGestionDesprets.place(x=0 , y=280) 

        
        self.ImageSedeconnecter = PhotoImage(file="Images/Sedeconnecter.png")
        self.BoutonPourSedeconnecter = Button(self.PageGestiondesprets,cursor="hand2", text="",command=self.PourSeDeConnecter,image=self.ImageSedeconnecter, width=184, height=90, bg="#8E7E23",font="arial 12 bold")
        self.BoutonPourSedeconnecter.place(x=0 , y=420) 

        

        #titres
        titreGestionprets = Label(self.PageGestiondesprets, text=" Gestion Prêts ",font =("algarian", 20,"bold"), bg="#8E7E23", fg="black")
        titreGestionprets.place(x=350, y=20,width=500)
        
        titreGestionlivres = Label(self.PageGestiondesprets, text=" Livres ",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        titreGestionlivres.place(x=0, y=100,width=190)

        titreAdherents = Label(self.PageGestiondesprets, text=" Adhérents ",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        titreAdherents.place(x=0, y=240,width=190)

        titreGestiondesprets = Label(self.PageGestiondesprets, text=" Prêts",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        titreGestiondesprets.place(x=0, y=380,width=190)

        titreSedeconnecter = Label(self.PageGestiondesprets, text=" Se déconnecter",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        titreSedeconnecter.place(x=0, y=520,width=190)

        titreRechercherlesemprunts= Label(self.PageGestiondesprets, text=" Rechercher les emprunts par :",font =("algarian", 12,"bold"), bg="#8E7E23", fg="black")
        ...
        BoutonPourRechercherUnEmprunt = Button(self.PageGestiondesprets,command=self.ClickBoutonRechercher, text="Rechercher  ",cursor="hand2", font=("times new roman",11), bd=0,bg="#8E7E23",fg="black")
        BoutonPourRechercherUnEmprunt.place(x=730, y=90) # command est attribuer a un bouton pour pouvoir l'executer  

        BoutonActualiser = Button(self.PageGestiondesprets,command=self.ClickActualiser, text="Actualiser ",cursor="hand2", font=("times new roman",11), bd=0,bg="#8E7E23",fg="black")
        BoutonActualiser.place(x=820, y=90) # command est attribuer a un bouton pour pouvoir l'executer  

        BoutonPourAjouterUnlivre = Button(self.PageGestiondesprets,command=self.PageEmprunt, text="Cliquer pour emprunter un livre",cursor="hand2", font=("times new roman",11), bd=0,bg="#8E7E23",fg="black", )
        BoutonPourAjouterUnlivre.place(x=215, y=500)

        
        BoutonRetournerUnLivre = Button(self.PageGestiondesprets,command=self.RestituerUnEmprunt, text="Restituer un emprunt",cursor="hand2", font=("times new roman",11), bd=0,bg="#8E7E23",fg="black")
    ...
    def PageEmprunt(self):          
        ...
        self.PageEmprunt.config(bg="#9D7B7F")  # background de la frame 
        ...
        #titres
        titrenom = Label(self.PageEmprunt, text=" idAdherent ",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        titrenom.place(x=6, y=50)

        titrelivre = Label(self.PageEmprunt, text=" idLivre ",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        titrelivre.place(x=10, y=100)

        self.date_emprunte = Label(self.PageEmprunt, text="dateEmprunt ",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        self.date_emprunte.place(x=10, y=200)

        date_retour = Label(self.PageEmprunt, text="dateRetour ",font =("algarian", 15,"bold"), bg="#8E7E23", fg="black")
        ...
        #bouton
        BoutonSuivant = Button(self.PageEmprunt, command=self.ClickBoutonSuivant, text="Suivant",font=("times new roman", 20),cursor="hand2", bg="#8E7E23").place(x=10, y=300, height=60, width=150)
    ...
    def ClickBoutonSuivant(self):
        ...
        try:
                ...
                execut=connectionbdd.cursor()
                execut.execute("select * from emprunter where numAdherent=%s",self.var_iddelhadherent.get())
                ...
                elif execut.execute("select * from emprunter where numLivre=%s",self.var_idlivre.get()):

                  messagebox.showerror("Erreur", "Ce livre à déja été emprunter, il faut choisir un autre livre", parent=self.PageEmprunt)
                
                else:
                
                 execut.execute("insert into emprunter (numAdherent,numLivre,dateEmprunt,dateRetour) values (%s,%s,%s,%s)",
