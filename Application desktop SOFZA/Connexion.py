...
class PagedeConnexion:  #classe formulaire 
    def __init__(self,root): #self représente l'instance de la classe. En utilisant le mot clé " self " nous pouvons accéder aux attributs et méthodes de la classe en python. 
        ...
        PaneauGrisDuHaut = Frame(self.PagedeConnexion, bg="#9D7B7F") #Frame est un conteneur qui permets de gérer des widgets paneau orange du haut 
        PaneauGrisDuHaut.place(x=0, y=-30, width=1100, height=150) #largeur longeur taille du panneau

        self.ImageDelaPageConnexion = PhotoImage(file="Images/Sofza.png") 
        ...
        PanneauOrangeDuMilieu = Frame(self.PagedeConnexion, bg="#ff7f00",) #Frame est Un cadre est simplement un conteneur pour d’autre widgets.
        PanneauOrangeDuMilieu.place(x=650, y=160, width=350, height=240) 
        

        PaneauGrisDuBas = Frame(self.PagedeConnexion, bg="#9D7B7F") #Frame est Un cadre est simplement un conteneur pour d’autre widgets.
        PaneauGrisDuBas.place(x=0, y=435, width=1100, height=190) 

       


        #les titres
        titrepourSOFZAPourLesLecteurs = Label(PaneauGrisDuHaut, text=" Bienvenue chez SOFZA", font =("algarian", 30,"bold"), bg="#8E7E23", fg="black")
        titrepourSOFZAPourLesLecteurs.place(x=260, y=60) 

        titrepourBySofiane = Label(PaneauGrisDuBas, text="Application realisé par Sofiane Zaion ", font =("Times New Roman", 15,"italic"), bg="#8E7E23", fg="black")
        titrepourBySofiane.place(x=460, y=45)
  
        titreSidentifier =  Label(PanneauOrangeDuMilieu, text="S'identifier", font =("Times New Roman", 15,"bold"), bg="#8E7E23", fg="black")
        titreSidentifier.place(x=148, y=16)

        titreEmail = Label(PanneauOrangeDuMilieu, text="E-mail:", font =("Times New Roman", 12,"bold",), bg="#8E7E23", fg="black")
        titreEmail.place(x=10, y=80)
        
        titreMotdepasse = Label(PanneauOrangeDuMilieu, text="Mot de passe:", font =("Times New Roman", 12,"bold"), bg="#8E7E23", fg="black")
        titreMotdepasse.place(x=10, y=120)
        
        
        #Entry c'est un champs de saisie en français
        self.emailDeLaPageConnexion= Entry(PanneauOrangeDuMilieu, font= (5), bg="white") 
        self.emailDeLaPageConnexion.place(x=150, y=80,width=150)


        self.motdepasseDeLaPageConnexion= Entry(PanneauOrangeDuMilieu, show="*",font= (5), bg="white")  # show="*" pour cacher le mot de passe
        ...
        BoutonCreeuncompte = Button(PaneauGrisDuHaut, text="Créer un compte",command=self.PageCreationDeCompte, cursor="hand2", font=("times new roman",12,"bold"), bd=0,bg="#8E7E23",fg="black")
        ...
        #Bouton mot de passe 
        BoutonMotdepasseoublie = Button(PanneauOrangeDuMilieu, text="Mot de passe oublié ?", command=self.CliquerMotdepasseOublie, cursor="hand2", font=("times new roman",11), bd=0,bg="#8E7E23",fg="black")
        ...
        #bouton connexion
        BoutonConnexion = Button(PanneauOrangeDuMilieu, text="Se connecter",command=self.CliquerBoutonConnexion, cursor="hand2", font=("times new roman",12,"bold"), bd=0,bg="#8E7E23",fg="black")
        ...
        if self.emailDeLaPageConnexion.get()=="": #si le champs email est null alors
            ...
            try:
                ...
                if ligne == None: #si les champs ne corresponde pas a ceux dans la base de données alors 
                    ...
                    self.PageDeMotdepasseOublie.config(bg="#8E7E23") #couleur du background 
    ...
    def MotdePasseOublie(self):
       if self.questionUtilisateur.get() == "" or self.questionDeLaPageMotDepasseOublie.get() == "" or self.nouveaumotdepasseDelaPageMotdepasseOublie.get() == "":
        messagebox.showerror("Erreur", "Veuillez remplir tous les champs obligatoires", parent=self.PageDeMotdepasseOublie)
       else:
        try:
            connectionbdd = pymysql.connect(host="localhost", user="root", password="", database="bddcomptes")
            execut = connectionbdd.cursor()
            execut.execute("SELECT * FROM utilisateur WHERE emailUtilisateur=%s AND questionUtilisateur=%s AND reponseUtilisateur=%s", (self.emailDeLaPageConnexion.get(), self.questionDeLaPageMotDepasseOublie.get(), self.questionUtilisateur.get()))
            ligne = execut.fetchone()

            if not ligne:  # Vérifie si ligne est vide
                messagebox.showerror("Erreur", "Vous n'avez pas bien répondu à la question sélectionnée", parent=self.PageDeMotdepasseOublie)
            else:
                ...
                connectionbdd.close()
                messagebox.showinfo("Succès", "Vous avez modifié votre mot de passe", parent=self.PageDeMotdepasseOublie)
    ...
    def PageCreationDeCompte(self): 
                    ...
                    self.PageCreationDeCompte.config(bg="#9D7B7F") #Couleur de la page 
                    ...
                    #Titres
                    titreDeLaPageCreeunCompte = Label(self.PageCreationDeCompte, text="Créer votre compte SOFZA", font =("Arial", 12,"bold"), fg="black")
                    titreDeLaPageCreeunCompte.place(x=100, y=20)

                    titreNom = Label(self.PageCreationDeCompte, text=" Nom : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    titreNom.place(x=10, y=80)
                    
                    titrePrenom = Label(self.PageCreationDeCompte, text=" Prénom : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    titrePrenom.place(x=10, y=120)

                    titreEmail = Label(self.PageCreationDeCompte, text=" Email : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    titreEmail.place(x=10, y=160)

                    titreTelephone = Label(self.PageCreationDeCompte, text=" Téléphone : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    titreTelephone.place(x=10, y=200)
                    
                    titreSelectionnezUnequestion = Label(self.PageCreationDeCompte, text=" Séléctionnez une question : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    titreSelectionnezUnequestion.place(x=10, y=240)

                    titreRepondre = Label(self.PageCreationDeCompte, text=" Répondre : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    titreRepondre.place(x=10, y=280)
                    
                    TitreMotdepasse = Label(self.PageCreationDeCompte, text=" Mot de passe : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    TitreMotdepasse.place(x=10, y=320)

                    titreConfirmeeMotdepasse = Label(self.PageCreationDeCompte, text=" Confirmée mot de passe : ", font =("algarian", 11,), bg="#8E7E23", fg="black")
                    ...
                    BoutonSuivant = Button(self.PageCreationDeCompte, text="Suivant",command=self.CliquerBoutonCreeuncompte,cursor="hand2", font=("times new roman",12,"bold"), bd=0,bg="#8E7E23",fg="black")
